...
对于 MKV 视频，播放器会在浏览器本地尝试提取第一条文字字幕轨道，并显示 ASS、SSA 或 SRT 等文字字幕。第一次使用内嵌字幕功能时，需要加载浏览器端 FFmpeg 组件，可能需要等待一段时间；视频和字幕内容不会上传。PGS 等图像字幕轨道暂不支持。
